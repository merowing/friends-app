              ...
              switch(sortKey) {
                case 'name':
                  friendList.sort((firstFriend, secondFriend) => {
                      ...
                      const condition = (!+updown) ? firstName > secondName : secondName > firstName;
                ...
                case 'age':
                  friendList.sort((first, second) => {
                    return (!parseInt(updown)) ? first.dob.age - second.dob.age : second.dob.age - first.dob.age;
                  });
                  break;
              }
            }
...
function createCountryFilter() {
    ...
    const countryOptions = countries.map(item => `<option value=${item}>${item}</option>`);
